...
import {
  ...
  validationSchema,
  handleSubmit,
} from "./newPublicServiceRequestHelper";
...
const PublicRequestMain: React.FC = () => {
  ...
  function showCurrentStep(values: any) {
    ...
    if (isRequestSuccessful === null) {
      ...
      !isOpen
        ? (CurrentComponent = (
            <StepOneFields selectedCategory={values.category} />
          ))
      ...
      >
        {({ errors, values, isSubmitting }) => (
          <>
            ...
            >
              <Stack py="15px">{showCurrentStep(values)}</Stack>
...
const PublicRequestSide: React.FC = () => {
  return (
    ...
    >
      ...
      <Stack>
        <Text fontSize="sm">
          ¡Pide presupuesto gratis! Haz una solicitud indicando lo que
          necesitas, mientras más detalles incluyas más fácil será encontrar un
          profesional que se adapte a ti.
        </Text>
      </Stack>
    </Stack>
  );
...
const ErrorMessage: React.FC = () => {
  return (
    <Box textAlign="center" py={10} px={6}>
      ...
      <Button as="a" href="/request/new" mt="1rem">
        Regresar
      </Button>
    </Box>
  );
