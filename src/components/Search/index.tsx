...
function SliderInput({ filters, setFilters }: { filters: any, setFilters: any }) {
  ...
  return (
      ...
      >
        ...
        <SliderThumb fontSize='sm' boxSize={6} color='black' title='Rango (km)'>{value}</SliderThumb>
      </Slider>
  )
}
...
function RangeSliderInput({ filters, setFilters }: { filters: any, setFilters: any }) {
  ...
  return (
    <RangeSlider aria-label={['min', 'max']} defaultValue={[1, 5]} min={1} max={5} onChange={(val) => setValue(val)}>
      ...
      <RangeSliderThumb boxSize={6} color='black' index={0}>{value[0]}</RangeSliderThumb>
      <RangeSliderThumb boxSize={6} color='black' index={1}>{value[1]}</RangeSliderThumb>
    </RangeSlider>
  );
}